...
The goal of our project is to develop an Interactive Drought Dashboard that presents a detailed view of Arizona's drought conditions from 2000 to the present. This interactive tool allows users to examine drought trends over time and analyze the data for specific time frames or particular counties, providing a dynamic and user-focused experience. The goal includes:

-   Providing geographic visualizations, including maps showcasing Arizona's counties, allowing users to filter and analyze data at the county level.

-   Enable users to track trends over time within a particular county or to compare the severity of the drought across counties.

-   Create dynamic time series analysis, showcasing the evolution of drought severity over time, complemented by statistical insights through stacked bar plots and pie charts illustrating average drought levels across counties and years.
